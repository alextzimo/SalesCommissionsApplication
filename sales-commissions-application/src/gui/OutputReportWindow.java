...
public class OutputReportWindow extends JDialog {
	...
	private SalesFilterWindow selectionWindow;
	private SalesCalculator selectedAgent;
	...
	public OutputReportWindow(final SalesFilterWindow sw, SalesCalculator agent,double tSales,int tItems,float shirtS,float skirtS,float trousersS,float coatsS,double com) {
		selectionWindow = sw;
		selectedAgent = agent;
		totalSales = tSales;
		totalItems = tItems;
	...
	private void initialise(){
		...
		resultWindowPanel.setLayout(null);
		{
			...
			xmlReportButton.setBounds(436, 144, 163, 84);
		...
		JButton btnNewButton_2 = new JButton("\u0395\u03BE\u03B1\u03B3\u03C9\u03B3\u03AE \u03C3\u03B5 HTML");
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				outputHTMLButtonPressed(arg0);
			}
		});
		btnNewButton_2.setFont(new Font("Times New Roman", Font.PLAIN, 14));
		btnNewButton_2.setBounds(436, 265, 163, 83);
		resultWindowPanel.add(btnNewButton_2);
		updateResults();
	...
	private void outputTXTButtonPressed(ActionEvent evt) {
	    ...
	    fileChooser.setDialogTitle("Εξαγωγή σε TXT");
	    ...
	    if (userSelection == JFileChooser.APPROVE_OPTION) {
	        ...
	        TXTOutput makeTXTFile = new TXTOutput(selectedAgent);
	        makeTXTFile.writeToFile(filePath);
	        
	        JOptionPane.showMessageDialog(null, "Το αρχείο TXT αποθηκεύτηκε στο: " + filePath);
	    }
	}
	
	private void outputXMLButtonPressed(ActionEvent evt) {
		JFileChooser fileChooser = new JFileChooser();
	    fileChooser.setDialogTitle("Εξαγωγή σε XML");
	    ...
	    if (userSelection == JFileChooser.APPROVE_OPTION) {
	        ...
	        XMLOutput makeXMLFile = new XMLOutput(selectedAgent);
	        makeXMLFile.writeToFile(filePath);
	        
	        JOptionPane.showMessageDialog(null, "Το αρχείο XML αποθηκεύτηκε στο: " + filePath);
	    }		
	}
	
	private void outputHTMLButtonPressed(ActionEvent evt) {
		JFileChooser fileChooser = new JFileChooser();
	    fileChooser.setDialogTitle("Εξαγωγή σε HTML");
	    ...
	    if (userSelection == JFileChooser.APPROVE_OPTION) {
	        ...
	        HTMLOutput makeXMLFile = new HTMLOutput(selectedAgent);
	        makeXMLFile.writeToFile(filePath);
	        
	        JOptionPane.showMessageDialog(null, "Το αρχείο HTML αποθηκεύτηκε στο: " + filePath);
	    }		
	}
