...
public class SalesFilterWindow extends JDialog {
	...
	private ImportAgentsWindow inputDialog;
	private SalesCalculator selectedAgent;
	@SuppressWarnings("unused")
	private String fileType;
	
	public SalesFilterWindow(ImportAgentsWindow dialog, SalesCalculator agent, String fileTypeFlag) {
		inputDialog = dialog;
		selectedAgent = agent;
	...
	protected void okButtonPressed(ActionEvent evt) {
		...
		if (categoryCheckBox.isSelected()) {		
			if(shirtRadio.isSelected())
				shirtSales = selectedAgent.calculateClothesSales("Shirts");

			if(skirtRadio.isSelected())
				skirtSales = selectedAgent.calculateClothesSales("Skirts");
			
			if(coatRadio.isSelected())
				coatsSales = selectedAgent.calculateClothesSales("Coats");
			
			if(trousersRadio.isSelected())
				trousersSales = selectedAgent.calculateClothesSales("Trousers");
		}
		else {
			shirtSales = skirtSales = coatsSales = trousersSales = -1;
		}
		...
		if(commissionCheckBox.isSelected())
			...
			commission = -1;
		OutputReportWindow rs = new OutputReportWindow(this,selectedAgent, totalSales, totalItems, shirtSales, skirtSales, trousersSales, coatsSales, commission);
		rs.setVisible(true);
	...
	private void addReceiptButtonPressed(ActionEvent evt) {
		...
		checkList.add(receiptIDTextField.getText());
		checkList.add(dateTextField.getText());
		checkList.add(salesTextField.getText());
		checkList.add(kindTextField.getText());
		checkList.add(itemsTextField.getText());
		checkList.add(companyTextField.getText());
		checkList.add(countryTextField.getText());
		checkList.add(cityTextField.getText());
		checkList.add(streetTextField.getText());
		checkList.add(numberTextField.getText());
